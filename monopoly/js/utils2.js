		...
		move : function(moveId, positionIndex){
			...
			function moving(){
				...
				if(_moveCount == moveCount){
					...
					// x축으로 움직일경우
					if(sPosition.x != ePosition.x){
						index = Math.abs(sPosition.x + ePosition.x * -3);
						movePosition = {x: index};

						// x축으로 움직일때
						if(typeof movePosition.x === "number"){
							if(sPosition.x > ePosition.x){
								obj.object3D.position.x += (movePosition.x * -3) / 12;
							}else{
								obj.object3D.position.x += movePosition.x / 12;
							}
						}
					}
					...
					// z축으로 움직일경우
					if(sPosition.z != ePosition.z){
						index = Math.abs(sPosition.z + ePosition.z * -3);
						movePosition = {z: index};

						// z축으로 움직일때
						if(typeof movePosition.z === "number"){
							if(sPosition.z > ePosition.z){
								obj.object3D.position.z += (movePosition.z * -3) / 12;
							}else{
								obj.object3D.position.z += movePosition.z / 12;
							}
						}
					}
