				...
				if(_moveCount == moveCount){
					...
					if(sPosition.x != ePosition.x){
						...
						if(typeof movePosition.x === "number"){
							if(sPosition.x > ePosition.x){
								obj.object3D.position.x = parseFloat(parseFloat(obj.object3D.position.x) + parseFloat(parseFloat(((movePosition.x * -1) / 4) / 5).toFixed(3))).toFixed(3);
							}else{
								obj.object3D.position.x = parseFloat(parseFloat(obj.object3D.position.x) + parseFloat(parseFloat((movePosition.x / 4) / 5).toFixed(3))).toFixed(3);
							}
						}
					}
					...
					if(sPosition.z != ePosition.z){
						...
						if(typeof movePosition.z === "number"){
							if(sPosition.z > ePosition.z){
								obj.object3D.position.z = parseFloat(parseFloat(obj.object3D.position.z) + parseFloat(parseFloat(((movePosition.z * -1) / 4) / 5).toFixed(3))).toFixed(3);
							}else{
								obj.object3D.position.z = parseFloat(parseFloat(obj.object3D.position.z) + parseFloat(parseFloat((movePosition.z / 4) / 5).toFixed(3))).toFixed(3);
							}
						}
					}
...
//						alert('movingIndex : '+ movingIndex);
		...
		cameraOn : function(cameraId){
			...
			if(typeof cameraId === "undefined"){
				cameraId = movingUserId + "Camera";
				this.objView(movingUserId + "ViewBackBtn", false);
			}else{
				this.objView(movingUserId + "ViewBackBtn", true);
			}
