...
var KFRAME = {
    registerComponent : function(name, component) {
		...
		component.init = function () {
			...
			/*this.el.addEventListener('broadcast', function (event) {
			...
			// this.el.addEventListener("BroadcastEvent", this.onBroadcastEvent.bind(this));
			...
			if (this.onAddEventListener) this.onAddEventListener();
		};
...
document.addEventListener("DOMContentLoaded", function(event) {
    AFRAME.scenes[0].addEventListener("loaded", function(event) {
        //inspector inject 하기 싫으면 주석 하면 된다.!!!
        // setTimeout(function(){ AFRAME.scenes[0].components["inspector"].injectInspector(); }, 1000);
        //setTimeout(function(){ AFRAME.scenes[0].components["inspector"].injectInspector(); }, 500);
        // AFRAME.scenes[0].components["inspector"].injectInspector();
    });
});
var KEvent = {
    ...
    sendEvent : function(name, targetId) {
    	if(targetId != null && targetId != undefined && targetId != "") {

    		document.getElementById(targetId).emit(name, null, false);
    	} else {
    		document.querySelector('a-scene').emit(name, null);
    	}
    },
    broadcast : function(targetEventName, targetId) {
    	//console.log('broadcast 호출');
        document.querySelector('a-scene').emit('broadcast', {eventName : targetEventName, targetId: targetId}, false);
    },
    call : function(targetEventName) {
        document.querySelector('a-scene').emit('eventlisten',{eventName : targetEventName}, false);
    },
    componentCall : function(targetEventName, targetId) {
    	//console.log('componentCall 호출');
        if(targetId != null && targetId != undefined && targetId != "") {
        	//console.log("11111");
            //console.log("tragetId : " + targetId);
            KEvent.emit('eventlisten', {eventName: targetEventName, targetId: targetId}, false);
        } else {
			//console.log("여기까지");
            for(var i = 0; i < CustomComponentList.length; i++) {
                var componentName = CustomComponentList[i].componentName;
                //if(targetEventName == componentName){
                var entities = document.querySelectorAll("a-entity["+componentName+"]");
                //console.log("componentName : " + componentName);
                //console.log("length : " + entities.length);
                for(var j = 0; j < entities.length; j++) {
                	//console.log("몇번 호출 ? " + (j+1));
					//console.log("eventName : " + targetEventName + " / targetId : " + entities[j].id);
                    KEvent.emit('eventlisten', {eventName: targetEventName, targetId: entities[j].id}, false);
                	}
                //}
            }
        }
